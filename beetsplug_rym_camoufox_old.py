...
class RYMCamoufoxPlugin(plugins.BeetsPlugin):
    ...
    async def _process_single_album_async(self, album: Album, page, dry_run: bool = False):
        ...
        try:
            ...
            genre_data = await self._extract_genres_from_url_async(direct_url, page)
            genres = genre_data.get('genres', [])
            descriptors = genre_data.get('descriptors', [])
            ...
            if not genres:
                ...
                genre_data = await self._extract_genres_from_url_async(album_url, page)
                genres = genre_data.get('genres', [])
                descriptors = genre_data.get('descriptors', [])
    ...
    async def _search_album_url_async(self, search_url: str, page, artist: str, album: str, year: Optional[int] = None) -> Optional[str]:
        ...
        try:
            ...
            self._log.info(f"Found {len(result_tables)} search result tables")
            ...
            for i, table in enumerate(result_tables):
                ...
                if candidate_info:
                    ...
                    if album_link:
                        ...
                        self._log.info(f"Result {i}: {candidate_info['artist']} - {candidate_info['album']} ({candidate_info['year']}) Score: {score:.3f}")
                    else:
    ...
    async def _extract_genres_from_url_async(self, url: str, page) -> dict:
